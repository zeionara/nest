...
public func sample<InputType: Randomizable, OutputType: Numeric>(
    ...
    await sample(
        getProbability, nSamples, nParts: nParts, from: from, to: to, precision: precision,
        kind: kind, generatorKind: generatorKind, seed: seed
    ) {
        DefaultSeedableRandomNumberGenerator($0)
    }
}
